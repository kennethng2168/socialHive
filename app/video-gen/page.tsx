...
const videoModels: VideoModel[] = [
  ...
  // Self-Hosted Model
  {
    ...
    description: "Your own self-hosted video generation model for complete control",
    speed: "Standard",
    resolution: ["720p", "1080p"],
    duration: ["5s", "10s", "15s"],
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (selectedModel.id === "amazon-nova-reel") {
        ...
        response = await fetch('/api/nova-reel', {
          method: 'POST',
          body: formData
        });
      }
      // Route to WaveSpeedAI for other models
      else {
        const formData = new FormData();
        ...
        response = await fetch('/api/wavespeed-video', {
          method: 'POST',
          body: formData
        });
      }
